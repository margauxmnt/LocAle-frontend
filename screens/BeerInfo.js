...
export default function BeerInfo({ navigation }) {
    ...
    useEffect(() => {
        const getSellers = async () => {
            const request = await fetch(`http://172.16.190.135:3000/get-sellers/${JSON.stringify(currentPosition)}/${beerInfo._id}`)
    ...
    const goBack = async () => {
        const request = await fetch(`http://172.16.190.135:3000/get-brewery-from-beer/${beerInfo._id}`)
        const result = await request.json()
    ...
    const addNote = async () => {
        const request = await fetch('http://172.16.190.135:3000/users/add-note', {
            method: 'POST',
