...
function TypeaheadUsingComponentWithRenderItem(props) {
    ...
    useEffect(() => {
        (async () => {
            const request = await fetch('http://192.168.1.24:3000/get-beers-n-notes')
    ...
    const selectSearch = (item) => {
        if (data[item].note !== undefined) {
            (async () => {
                const request = await fetch(`http://192.168.1.24:3000/get-beer/${data[item].id}`)
                const result = await request.json()
            ...
            (async () => {
                const request = await fetch(`http://192.168.1.24:3000/get-brewery/${data[item].id}`);
                const result = await request.json()
